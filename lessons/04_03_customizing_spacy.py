...
def train_spacy(data, iterations):
    """Train spacy huh."""
    TRAIN_DATA = data
    # tell spacy that we want to create a fresh english blank model
    nlp = spacy.blank("en")

    # we need to add ner into the nlp pipelines!
    if "ner" not in nlp.pipe_names:
        ner = nlp.create_pipe("ner")
        ## we created the pipe so now we will add it to the pipeline
        nlp.add_pipe(ner, last=True)
    # TODO: get why this
    for _, annotations in TRAIN_DATA:
        for ent in annotations.get("entities"):
            # get the label(in our case it's PERSON)
            ner.add_label(ent[2])

    # TODO: get why this?
    other_pipes = [pipe for pipe in nlp.pipe_names if pipe != "ner"]

    # TODO: *other_pipes stands and is used for what?
    with nlp.disable_pipes(*other_pipes):
        optimizer = nlp.begin_training()
        for itn in range(iterations):
            print ("Starting iteration " + str(itn))
            # needed because otherwise the model might memorize the order
...
def launch_training_and_save_model():
...
def clean_text(text):
    # it just works!
    # it's used for cleaning some text!
    # maybe use it at some point becaues it's useful for cleaning
    cleaned = re.sub(r"[\(\[].*?[\)\]]", "", text)
    return (cleaned)

def main():
    # here we just test our model
    # if you didnt run the launch_training_and_save_model()
    # you need to in order to train the model and save it.
